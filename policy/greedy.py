...
class GreedyPolicy(CleanPolicy):
	...
	:param: eco economic mode 	if set, we assume the agent 
			knows whether dirt comeback or not, and accordingly
			it may stop checking rooms after visiting them all. 
			By default, we assume the agent doesn't know if dirt 
			will re-appear.
	"""
	...
	Select an action according to: current state, world map and policy
	"""
	def select_action(self, state):
		room = state['agent']			# agent current location
		dirty = state['dirt']			# is there dirt in current room?
		# call user-defined algo
		action = self.agent_program(room, dirty)
		return action

	""" 
	'greedy' policy implemented as
	an agent function (program). 
	typically, a reflex-based agent with model.
	...
	"""
	Updates the last room the vaccuum was in,
	...
	"""
	Greedy agent program for 'vacuum-2rooms' world
	"""
	...
	"""
	Greedy agent for a vacuum cleaner robot without a location sensor, 
	...
	"""
	greedy agent for 4rooms_v0 world,
	see map in: testme.py, -|-
	"""
