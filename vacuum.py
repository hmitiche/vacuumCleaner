...
'vacuum.py'
Some util function to:
save simulation output, plot results and lauch cleaning policy.
Hakim Mitiche
March 2024
"""
from policy.base import RandomPolicy
from policy.qlearning import QLearnerPolicy
from policy.greedy import GreedyPolicy
from policy.greedyrandom import GreedyRandomPolicy
...
"""
Tools to: load a cleaning policy, save and plot simulation results.
"""
class tools:
	
	logger = None
	"""
	Instantiate a given cleaning policy
	"""
	@staticmethod
	def make_policy(policy_id, world_id, env, eco_mode):
		...
		match (pdict[policy_id]):
			...
			case 3:	p = QLearnerPolicy(world_id, env)
			case _:
...
if __name__ == '__main__':
	help_msg = f"""
	Usage:	python vacuum.py -h 
			display this help.
		python vacuum.py -v world_id [-s] [-a]
			show a comparaison plot of vacuum cleaning policies for 
			world_id and/or save its to figure: 'Figure_[world_id]_[metric].png'
